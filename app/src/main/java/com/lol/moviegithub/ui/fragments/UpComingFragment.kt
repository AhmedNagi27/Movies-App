...
import com.lol.movies.databinding.FragmentUpComingBinding
...
class UpComingFragment : Fragment() {
  ...
  override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
    ...
    binding.upComingRecyclerView.layoutManager = GridLayoutManager(requireContext(), 2)
    binding.upComingRecyclerView.adapter = adapter

    viewModel.uiState.observe(viewLifecycleOwner) { state ->
      when (state) {
        is UiState.Loading -> binding.uProgressBar.visibility = View.VISIBLE
        is UiState.Success -> {
          binding.uProgressBar.visibility = View.GONE
    ...
    binding.btnPrev.setOnClickListener {
      if (currentPage > 1) {
        ...
        viewModel.fetchUpComing(currentPage)
      }
    }
