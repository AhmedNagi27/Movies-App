...
import com.lol.movies.databinding.FragmentTopRatedBinding
...
class TopRatedFragment : Fragment() {
  ...
  override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
    ...
    viewModel.uiState.observe(viewLifecycleOwner) { state ->
      when (state) {
        is UiState.Loading -> binding.tProgressBar.visibility = View.VISIBLE
        is UiState.Success -> {
          binding.tProgressBar.visibility = View.GONE
    ...
    binding.btnPrev.setOnClickListener {
      if (currentPage > 1) {
        ...
        viewModel.fetchTopRated(currentPage)
      }
    }
